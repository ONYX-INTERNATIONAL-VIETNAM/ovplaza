...
const FEATURES: Record<string, { tagline: string; bullets: string[] }> = {
  ovvideo: { tagline: "Nền tảng video & livestream tương tác, vé và quảng cáo.",
    bullets: [
  ...
  ovlab: { tagline: "Giám sát thử nghiệm vật liệu theo thời gian thực — website riêng tư cho nội bộ.",
    bullets: [
  ...
  ovcoffe: { tagline: "Mạng live video chat: tạo phòng, trò chuyện, chia sẻ đa phương tiện.",
    bullets: [
  ...
  ovtutor: { tagline: "Nền tảng học trực tuyến như Udemy nhưng hỗ trợ lớp học livestream.",
    bullets: [
  ...
  ovpay: { tagline: "Thanh toán an toàn trong hệ sinh thái OVPlaza (ví, thẻ, chuyển khoản).",
    bullets: [
...
export default function InfoDrawer({
  ...
  const open = !!module;
  const content = useMemo(() => (module ? (FEATURES[module.key] ?? { tagline: module.desc ?? "", bullets: [] }) : null), [module]);
        ...
        <>
          ...
          >
            <div className="relative flex h-full flex-col rounded-l-3xl border-l border-white/10 bg-[#0b0a20]/95 shadow-2xl">
              ...
              {/* NÚT ĐÓNG – nổi bật, tương phản cao */}
              <motion.button
                onClick={onClose}
                whileHover={{ scale: 1.06 }}
                whileTap={{ scale: 0.96 }}
                className="group absolute right-5 top-5 z-20 grid h-10 w-10 place-items-center rounded-full
                           bg-white text-[#0b0a20] shadow-[0_10px_28px_rgba(0,0,0,.35)]
                           ring-2 ring-white/80 hover:ring-4"
                aria-label="Đóng (Esc)" title="Đóng (Esc)"
              >
              ...
              <div className="relative p-6 pb-3">
                ...
                >
                  <span className="inline-block h-2 w-2 rounded-full" style={{ background: module.color }} />
                  OVPLAZA • MODULE
                </div>

                <div className="mt-4">
                  <h2 className="text-3xl font-extrabold tracking-tight text-white drop-shadow">{module.label}</h2>
                  <p className="mt-2 max-w-prose text-sm text-white/75">{content.tagline}</p>
                </div>

                {/* dải xiên (nhạt hơn để không nuốt nút) */}
                <div
                  className="pointer-events-none absolute right-0 top-0 h-24 w-40 bg-white/70"
                  style={{ clipPath: "polygon(10% 0%, 100% 0%, 100% 100%, 55% 100%)", zIndex: 5 }}
                />
              </div>
                  ...
                  <ul className="space-y-2">
                    {content.bullets.map((b, i) => (
                      <li key={i} className="flex items-start gap-3 text-[13px] text-white/85">
                        <svg width="18" height="18" viewBox="0 0 24 24" className="mt-0.5 flex-none opacity-80" style={{ color: module.color }}>
                          <path fill="currentColor" d="M12 2a10 10 0 1 1-7.07 2.93A10 10 0 0 1 12 2m-1.1 13.77l6-6-1.41-1.41-4.59 4.59-2.12-2.12-1.41 1.41 3.53 3.53Z"/>
                        </svg>
                        <span>{b}</span>
                      </li>
                    ))}
